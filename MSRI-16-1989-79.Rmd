    ...
    \oldpage{102}
    We have $E\otimes\QQ_p=\bigoplus_{v\mid p}E_v$, and $N_\crisp$ is the sum of the $M_\crisv$.
    ...
    A filtration $W$ of the objects [(M'1)E](minus-three-points-mp1e), [(M'2)E](minus-three-points-mp2e), [(M'3)E](minus-three-points-mp3e), and [(M'4)E](minus-three-points-mp4e), respected by the comparison isomorphisms.
    :::
...
::: {.rmenv #chunk-1.19 title="1.19" latex="{1.19}"}
    With the definition \ref{minus-three-points-1.17} of smooth realisation systems over $S=\Spec(\OO)$, the functor given by forgetting the $A$-module structure, which takes values in smooth realisation systems over $\Spec(\ZZ)\setminus P$, is called the "\em{direct image of $S$ in $\Spec(\ZZ)\setminus P$}".
    In the various realisations of \ref{minus-three-points-1.18}, it corresponds to the direct image.
    Its left adjoint, $M\mapsto A\otimes M$ (cf. \fref{minus-three-points-5.3}), is the \em{inverse image}.
:::

::: {.rmenv #chunk-1.20 title="1.20" latex="{1.20}"}
    If $S$ is an open of the spectrum of the ring of integers of a finite extension $E$ of $\QQ$, then we can modify the description in \ref{minus-three-points-1.18} as follows to define smooth realisation systems over $S$.
    ...
    - In [(M'4)E](minus-three-points-mp4e), for each residue field $k(v)$ of $S$, we ask for $M_\crisv$ over the field of fractions $K_v$ of Witt vectors over $k(v)$, endowed with a semi-linear $\phi_v$.
      The crystalline comparison isomorphism of [(M'5)E](minus-three-points-mp5e) then becomes
      $$
...
:::: {.rmenv #chunk-1.21 title="1.21" latex="{1.21}"}
    ...
    Here is a provisional definition of smooth realisation systems over $S$, inspired by \ref{minus-three-points-1.18}.
    ...
    A filtration $F$ of $M_\DR$ by vector sub-bundles: the Hodge filtration.
    ...
    A filtration $W$ of the objects [(M'1)S](minus-three-points-mp1s) to [(M'4)S](minus-three-points-mp4s), respected by the comparison isomorphisms.
    :::
...
:::: {.rmenv #chunk-1.24 title="1.24" latex="{1.24}"}
    ...
    This causes us to modify [(M'1)S](minus-three-points-mp1s), [(M'3)S](minus-three-points-mp3s), and [(M'5)S](minus-three-points-mp5s) as follows.
