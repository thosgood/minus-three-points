    ...
    As a catch-all, this category does the job (cf. \ref{minus-three-points-1.13} nonetheless).
    Additional axioms will always be natural, notably concerning the behaviour at infinity of the variation of mixed Hodge structures $H_\BB$ (cf. [[SZ](SZ)]).
::::

::: {.rmenv #chunk-1.22 title="1.22 Definition" latex="{1.22 Definition}"}
    \oldpage{104}
    Let $E$ be a finite extension of $\QQ$.
    Then a realisation system \em{with coefficients in $E$} is a realisation system $M$ endowed with the structure of an $E$-modules, $E\to\End(M)$.
:::
...
::: {.rmenv #chunk-1.23 title="1.23 Definition" latex="{1.23 Definition}"}
    A realisation system $M$ over $\QQ$ (resp. $\Spec\ZZ\setminus P$) \em{with integer coefficients} is a realisation system, denoted by $M\otimes\QQ$, endowed with a lattice $M_\BB\subset(M\otimes\QQ)_\BB$ such that, for all $\ell$, $M_\ell\coloneqq M_\BB\otimes\ZZ_\ell\subset(M\otimes\QQ)_\ell$ is stable under $\Gal(\bar{\QQ}/\QQ)$.
:::

:::: {.rmenv #chunk-1.24 title="1.24" latex="{1.24}"}
    On a more general base $S$, a realisation system \em{with integer coefficients} is a realisation system $M\otimes\QQ$ endowed with a local system of torsion-free $\ZZ$-modules $M_\BB\subset(M\otimes\QQ)_\BB$ satisfying $M_\BB\otimes\QQ\simto(M\otimes\QQ)_\BB$ (a "lattice") and such that the $\ZZ_\ell$-sheaf $M_\BB\otimes\ZZ_\ell$ on $S(\CC)$ corresponds, under $\comp_{\ell,\BB}$, to a smooth $\ZZ_\ell$-sheaf $M_\ell\subset(M\otimes\QQ)_\ell$ on $S[1/\ell]$.
...
::: {.rmenv #chunk-1.25 title="1.25" latex="{1.25}"}
    In \ref{minus-three-points-1.24}, the data of the $M_\ell$ is equivalent to that of a projective system of sheaves of $\ZZ/(n)$-modules on the $S[1/n]$:
    $$
...
::: {.rmenv #chunk-1.26 title="1.26" latex="{1.26}"}
    Combining the variants in \ref{minus-three-points-1.22} and \ref{minus-three-points-1.24}, we similarly define motives over $S$ with coefficients in the ring of integers of a finite extension of $\QQ$.
:::
...
Here is its description as a realisation system, in the language of \ref{minus-three-points-1.9} and \ref{minus-three-points-chunk-1.14} (along with \ref{minus-three-points-1.25}):
